...
def line_exit_check_and_set(loop, track, tp1, tp2, line_start, line_end, line_side):
    if isIntersect(tp1, tp2, line_start, line_end):
...
def engine(loopfile, cmd=False, custom_arg=None):
    global opt
    global count_boxes
    ...
    parser.add_argument('--project', default='static',
    ...
    parser.add_argument('--loop', default="loop.json",
                        type=str, help='loop setting file')
    parser.add_argument('--loop-txt', action='store_true',
                        help='save history for each loop')
    parser.add_argument('--summary-txt', action='store_true',
                        help='save summary for each loop')  # todo later
